...
- [X] T127 [P] Create Helm chart structure in charts/actual-assist/ directory (Chart.yaml with metadata, values.yaml with defaults, templates/, README.md per FR-012)
- [X] T128 [P] Implement Chart.yaml in charts/actual-assist/Chart.yaml (name: actual-assist, version: 0.1.0, description, repository, home, maintainers)
- [X] T129 [P] Create values.yaml in charts/actual-assist/values.yaml (production defaults: backend/frontend replicas, image tags, registry, resource limits, ingress TLS)
- [X] T130 [P] Create secret template in charts/actual-assist/templates/secret.yaml (conditional: secrets.create, OPENAI_API_KEY, ACTUAL_SERVER_URL, ACTUAL_PASSWORD, ACTUAL_BUDGET_ID with support for external secrets)
- [X] T131 [P] Create configmap template in charts/actual-assist/templates/configmap.yaml (non-secret env vars: NODE_ENV, SYNC_INTERVAL_MINUTES, SQLITE_DB_PATH, PORT)
- [X] T132 [P] Create deployment template for backend in charts/actual-assist/templates/deployment.yaml (replicas, image pull policy, envFrom for configmap/secret, liveness/readiness probes /health /ready, resource requests)
- [X] T133 [P] Create deployment template for frontend in charts/actual-assist/templates/deployment.yaml (nginx config, image, env, service port 80→5173)
- [X] T134 [P] Create PVC template in charts/actual-assist/templates/pvc.yaml (storage class, size 20Gi for budget data and SQLite, mount paths /data for backend)
- [X] T135 [P] Create service templates in charts/actual-assist/templates/service.yaml (backend service port 3000, frontend service port 80, type: ClusterIP)
- [X] T136 [P] Create optional ingress template in charts/actual-assist/templates/ingress.yaml (conditional: enabled via values.ingress.enabled, cert-manager annotation, TLS)
- [X] T137 [P] Create Helm helpers template in charts/actual-assist/templates/_helpers.tpl (labels, selector, fullname, chart name, secretName helpers per Helm best practices)
- [X] T138 [P] Create charts/actual-assist/README.md with installation steps (helm install, helm values reference, external secret examples for Flux/SOPS/ExternalSecrets)
- [X] T139 [P] Removed (consolidated into values.yaml with production defaults)
- [X] T140 [P] Removed (consolidated into values.yaml with production defaults)
- [X] T141 [P] Create charts/actual-assist/.helmignore with patterns to exclude from packaging (.git, .gitignore, tests/)
- [X] T142 Validate Helm chart syntax in charts/actual-assist/ with `helm lint charts/actual-assist/` (verify no errors, warnings acceptable with justification)
- [X] T143 Test Helm chart dry-run with `helm install --dry-run actual-assist charts/actual-assist/` (verify manifests generated, no rendering errors)
